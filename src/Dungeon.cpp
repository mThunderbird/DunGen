...
#pragma region graph
	...
	map<int, vector<int>> listOfNeighbours;
	
	for (auto& itr1 : m_rooms)
	{

		listOfNeighbours.insert(make_pair(itr1.first, vector<int>{}));

		for (auto& itr2 : m_rooms)
		{
			if (collRectRectF(wrapF(itr1.second.drawable.rect),wrapF(itr2.second.drawable.rect)) && itr1.first != itr2.first)
			{
				listOfNeighbours.at(itr1.first).push_back(itr2.first);
				/*
				line temp;
				temp.start = itr1.second.getCenter();
				temp.finish = itr2.second.getCenter();
				graphLines.push_back(temp);
				*/
			}
		}
	}

	// start building min spanning tree with root - main room;
...
#pragma endregion
	...
	circle midcircle;
	midcircle.coordinates = DUNGEON_CENTER;
	midcircle.radius = midZone;
	sectorCircles.push_back(midcircle);

	circle outercircle;
	outercircle.coordinates = DUNGEON_CENTER;
	outercircle.radius = outerZone;
